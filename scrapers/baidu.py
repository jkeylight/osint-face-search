...
class BaiduScraper(BaseScraper):
    """Baidu image search via Playwright"""
    ...
    async def search(self, image_path: str) -> List[SearchResult]:
        ...
        try:
            ...
            await page.goto("https://graph.baidu.com/pcpage/index?tpl_from=pc", wait_until="networkidle")
    ...
    async def _extract_results(self, page) -> List[SearchResult]:
        results = []
        try:
            items = await page.query_selector_all('.imgitem, .graph-similar-item, .card-wrap')
                    ...
                    if link:
                        ...
                        if href and href.startswith('http'):
                            ...
                            results.append(SearchResult(
                                url=href, source_url="https://baidu.com",
                                title=(title or "")[:100], engine="baidu",
                                thumbnail_url=thumbnail
                            ))
                except: continue
        except Exception as e:
            logger.error(f"Baidu extraction failed: {e}")
